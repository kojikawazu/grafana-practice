...
適用するスタック別ルール（本文は `.claude/rules/` が正本）:

- `.claude/rules/typescript.md`（`apps/task-app/**` 全体）
- `.claude/rules/frontend.md`（`pages` `components` `composables` `repositories` `utils` `types` `constants` `app.vue`）
